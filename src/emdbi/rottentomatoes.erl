...
handle_call({search, movie, Data, Options}, State) ->
  lager:info("[rottentomatoes] search movie ~p with ~p", [Data, Options]),
  {ok, [], State}; % TODO
handle_call({search, tv, Data, Options}, State) ->
  lager:info("[rottentomatoes] search tv ~p with ~p", [Data, Options]),
  {ok, [], State}; % TODO
handle_call({search, season, Data, Options}, State) ->
  lager:info("[rottentomatoes] search season ~p with ~p", [Data, Options]),
  {ok, [], State}; % TODO
handle_call({search, cast, Data, Options}, State) ->
  lager:info("[rottentomatoes] search cast ~p with ~p", [Data, Options]),
  {ok, [], State}; % TODO
handle_call({search, person, Data, Options}, State) ->
  lager:info("[rottentomatoes] search person ~p with ~p", [Data, Options]),
  {ok, [], State}; % TODO
handle_call({search, album, Data, Options}, State) ->
  lager:info("[rottentomatoes] search album ~p with ~p", [Data, Options]),
  {ok, [], State}; % TODO
handle_call({search, song, Data, Options}, State) ->
  lager:info("[rottentomatoes] search song ~p with ~p", [Data, Options]),
  {ok, [], State}; % TODO
